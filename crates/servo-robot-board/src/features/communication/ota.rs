...
impl OtaWriter {
    ...
    pub fn write_block(&mut self, flash: &FLASH, data: &[u8]) -> OtaWriteResult {
        // 边界检查
        if self.offset + data.len() as u32 > platform::flash::OTA_TEMP_MAX_SIZE {
        ...
        if !self.erased {
            ...
            if platform::flash::erase_ota_temp(flash).is_err() {
        ...
        // 写入 Flash（需要 4 字节对齐）
        let write_addr = platform::flash::OTA_TEMP_ADDR + self.offset;
        let aligned_len = data.len() & !3;

        if aligned_len > 0 {
            if platform::flash::program_flash(flash, write_addr, &data[..aligned_len]).is_ok() {
...
pub fn start_ota(flash: &FLASH) -> bool {
    ...
    if platform::flash::write_ota_flag(flash, platform::flash::OtaFlag::Pending).is_ok() {
        true
    } else {
